...
class TitleCrawler(gcrawl.Crawl):
    ...
    @staticmethod
    def crawl(job):
        c = TitleCrawler(job['seed'],job['allow_subdomains'],job['max_pages'])
        results = c.run()
        job.complete()
    ...
    def got(self,page):
        ...
        if page.status in (301, 302, 303, 307):
            ...
            if page.url not in self.bloomfilt:
                ...
                #use set difference to exclude links already in the bloomfilter
                new_links = set(page.links['follow'])-set(self.bloomfilt.contains(page.links['follow']))
                #self.extend(page.links['follow'], page)
                self.extend(new_links, page)
            else:
